/**
 * Accelerometer step detection — pure and testable, no sensor coupling.
 *
 * Peak-picking over the magnitude of acceleration (including gravity): a step is
 * a peak that rises above a smoothed baseline by `peakThreshold`, debounced by
 * `minIntervalMs`. Approximate by design (screen-on, accelerometer); a hardware
 * step counter would be a separate source behind the same pedometer port.
 */
...
export function cadenceFromSteps(stepTimesMs: number[], now: number, windowMs = 10_000): number {
  const cutoff = now - windowMs;
  const recent = stepTimesMs.filter((t) => t >= cutoff);
  if (recent.length < 2) return 0;
  return Math.round((recent.length / windowMs) * 60_000);
}
...
export class StepDetector {
  ...
  /** Feed one acceleration magnitude (m/s²) at time `t` (ms). True if a step counted. */
